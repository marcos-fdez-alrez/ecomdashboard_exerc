...
public class DashboardServiceImpl implements DashboardService {
    ...
    public HashMap<String, Object> getOrderDashboard() {
        ...
        final List<OrderRecieved> orderEntities = orderRecievedRepository.findAll();

        List<String> labels = new ArrayList<>();
        List<Double> orders = new ArrayList<>();

        for (OrderRecieved orderEntity : orderEntities) {
            labels.add(orderEntity.getDateReceived());
            orders.add(Double.valueOf(orderEntity.getOrderReceived()));
        }
    ...
    @Override
    public HashMap<String, Object> getBestCategoryDashboard() {
        HashMap<String,Object> orderDash = new HashMap<>();

        final List<ProductCategory> categoriesEntities = productCategoryRepository.findAll();

        List<String> labels = new ArrayList<>();
        List<Double> orders = new ArrayList<>();

        for (ProductCategory categoryEntity : categoriesEntities) {
            labels.add(categoryEntity.getCategoryName());
            orders.add(Double.valueOf(categoryEntity.getPercentage()));
        }

        orderDash.put("name",labels);
        orderDash.put("pct",orders);
